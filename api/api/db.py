...
from .settings import states, cities, parameters, stations
...
def _city_state(city:str) -> str:
    ...
    """
    queried = cities.find_one(dict(citiesInState=
    {'$elemMatch':dict(name=city)}))
...
def get(**kwargs) -> Dict[dict, str]:
    payload = construct_payload(**kwargs)
